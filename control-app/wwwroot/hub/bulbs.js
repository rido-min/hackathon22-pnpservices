...
const webSocket = new window.WebSocket(protocol + window.location.host)
  ...
  updateReportedUI(twin.properties.reported)
  ...
  webSocket.onmessage = (message) => {
    ...
    if (messageData.IotData.properties &&
        messageData.IotData.properties.reported &&
        messageData.IotData.properties.reported.lightState) {
      Vue.set(app, 'battLife', messageData.IotData.properties.reported.lightState.value)
      Vue.set(app, 'replacedAgo', moment(app.replaced).fromNow())
    }

    if (Number.isInteger(messageData.IotData)) {
      Vue.set(app, 'battLife', messageData.IotData)
    }
